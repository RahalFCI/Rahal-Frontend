...
| 2026-06-21 | Explorer **Level is derived on the frontend** from `cumulativeXp` via a single `XP_PER_LEVEL` constant (`src/shared/gamification/leveling.ts`) | The backend never computes Level from XP: `ExplorerProfile.Level` defaults to `1` and only changes via an admin endpoint — there is no XP→Level logic or threshold endpoint in `Gamification.*`. Deriving level client-side with a fixed, tunable threshold makes the level display meaningful and the level-up "Relic Title" moment (§3.2, §1.4 #3) demonstrable with zero backend dependency. If the backend later adds authoritative leveling, swap the derivation for the real `Level` field. |
| 2026-06-29 | **Demo fixtures** for places behind `flags.mockData` (`src/features/places/fixtures/`, `EXPO_PUBLIC_MOCK_DATA`, on by default in dev) | The dev backend has no seeded places, so the map/clustering/detail had nothing to render. The places API functions (list/byId/byCategory/search/categories/photos/reviews) short-circuit to curated Egyptian relics with real coordinates; fixtures satisfy the same Zod schemas so the client boundary is unchanged. Set `EXPO_PUBLIC_MOCK_DATA="false"` to force the real API. |
| 2026-06-29 | **Fog of war** discovery veil on the Discover map (`src/shared/map/fog.ts`) | Realises the deferred map-theming intent (§3.4) as an exploration mechanic: a pale "unexplored" veil over Egypt, with soft windows cleared around the explorer's visited places. Same polygon-with-holes technique as `egyptMask`, applied inside Egypt and feathered via three stacked equal-opacity fill bands. Reveal state = check-in history `placeId` joined to place coordinates (mock-seeded when `flags.mockData` and no real history). Toggle via the `fogEnabled` VectorMap prop. |
| 2026-06-29 | Discover-map markers are **custom `Marker` view-pins + client-side clustering**, superseding the native GeoJSON cluster source (§2.5) on this screen | Native vector clustering can't style markers as editorial "relic beacon" pins (Lucide category glyphs, amber/ghost/selected states). View annotations can, but don't use the native cluster source, so clustering is reimplemented as a small zoom-aware grid (`src/shared/map/clustering.ts`, `MapPins.tsx`). The `MapProvider` abstraction is unchanged; the swap is internal to `VectorMap`. |
| 2026-07-02 | **Notifications** (Phase 8 / §2.8) integrated behind `flags.push` (`EXPO_PUBLIC_PUSH`, on by default in dev): in-app center + unread badges (`src/features/notifications/`) driven by **polling** (no realtime — backend has no SignalR), plus **FCM push via the native device token**. | The backend Notifications module pushes through **Firebase Admin with a raw FCM token**, not Expo Push — so registration sends `Notifications.getDevicePushTokenAsync()` (the native FCM token on Android) to `POST /notifications/fcm-token`, requiring `google-services.json` baked into the dev client (`app.json android.googleServicesFile`, gitignored). Registration is best-effort/idempotent, wired via a single reactive effect in `NotificationsBridge` (root layout) keyed on the access token — covers app-start restore + email/Google login — so no per-auth-hook edits. Unread count polls every 30s (`useUnreadCount`); mark-read/all are optimistic. Backend gaps we design around, **not** worked around client-side: only 4 social notification types exist (no check-in/reward/level-up); `Social.Follow` exposes no `targetId`/actor so it can't deep-link to the follower (falls back to the list); one FCM token per user with **no DELETE endpoint**, so logout leaves a stale token; no preferences/opt-out. |
| 2026-07-03 | Payment gateway = **Stripe** (`@stripe/stripe-react-native` PaymentSheet), **superseding the PayMob decision** (2026-04-13, §8) | Backend switched from PayMob to Stripe (Firebase-style Stripe Admin) and exposed the PaymentSheet contract in `react-native-stripe-paymentsheet-guide.md`. Stripe's managed sheet + the backend already returning the publishable key make the Phase 6 premium-upgrade demo turnkey. PayMob (§8 risk row) is retired; a secondary gateway (§11 Q3) remains open. |
| 2026-07-03 | **Payments** (Phase 6 / §4) integrated behind `flags.payment` (`EXPO_PUBLIC_PAYMENTS`, on by default in dev): "Pay with card" on premium tiers → Stripe **PaymentSheet** → premium. The card→premium link is **composed on the client**, not server-side. | The backend does not link a Stripe charge to premium: the only endpoint that returns a Stripe client secret is the **anonymous** `POST /api/payments/test-intent` (a generic charge helper, **un-enveloped** raw JSON — so `paymentsApi` uses the raw `publicAxiosInstance` seam, not `apiClient`), while `POST /api/Subscription/purchase (Visa)` grants premium **synchronously without a real charge** and returns no secret, and the Stripe webhook has **no consumer that flips premium**. So the Rewards screen composes them: `useCardCheckout` (payment feature) runs the real sheet via `test-intent` for the tier's `weeklyPrice`; on success `useActivatePremiumWithCard` (rewards) calls `purchaseSubscription(Visa)` to grant premium and `pollPremium`s the profile. Publishable key comes from the test-intent response (optional `EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY` override); `initStripe` re-inits with it before presenting. Native module → dev-client rebuild required. Backend follow-up (out of scope): a `PaymentHandled` consumer that grants premium only after real charge confirmation, and a per-subscription intent that returns its own client secret. |
| 2026-07-03 | **Coupons split from Premium into a vendor catalog + redeem QR.** The Rewards tab is now the pure **Coupons** tab (Ticket icon); **Premium** is its own 6th tab (`app/(explorer)/premium.tsx`, Gem icon). Coupons are grouped by the offering **vendor** (`useCouponsByVendor` + `VendorCouponGroup`), and a successful claim raises a **QR celebration** (`CouponRedeemedMoment`, `react-native-qrcode-svg`). The wallet gains status filters (All · Ready · Redeemed · Expired). | Backend reality (verified): Explorer **claims** a coupon (spends `Coupon.XpCost`, `Status→Claimed`, gets a `CPN-…` code); a **Vendor** later **redeems** it via the Vendor-only `POST /UserCoupon/redeem` (`Status→Redeemed`) — the only "visible to vendor" mechanism (no notification event; `CouponRedeemedEvent` never fires). So this Explorer app renders the code as a **QR** the vendor scans in their own tooling, and the wallet reflects `Redeemed` on refetch. Terminology: user's "pending" = backend `Claimed`, user's "redeemed" = backend `Redeemed`. `GetCouponDto` carries only `vendorId`; vendor names/logos are resolved client-side via `GET /VendorProfile/{id}` (which is `[Authorize]`, explorer-accessible — the old `vendorsApi` "Vendor/Admin-gated" comment was wrong). `react-native-qrcode-svg` renders on the already-linked `react-native-svg`, so it's JS-only — **no dev-client rebuild** (supersedes the "no new UI libs" guidance for QR-as-infra). The dev DB has no seeded coupons (creation is Admin-only), so demo coupons/vendors are seeded directly into the DB. Backend follow-up (out of scope): fire `CouponRedeemedEvent` → notify the vendor, and a vendor-facing redemption list/dashboard. |
...
6. Backend refresh endpoint → current `POST /api/auth/generate` is protected by `[Authorize]`, validates `AccessToken` twice, and does not rotate refresh tokens; backend follow-up required before expired-token refresh can be fully verified.
